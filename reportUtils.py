...
def getArgs():
    parser = argparse.ArgumentParser(
        description="Parse inputs to Find QA issues",
    ...
    inputs.add_argument(
        ...
        "--preference_file",
        required=False,
        help="path to preference file, default=./preference_files/default.txt",
    )
    inputs.add_argument(
        "-T",
        "--thresholds",
...
def getMetrics(
    ...
    if metricSource.upper() == "IRIS":
        ...
        SQLcommand = (
            "SELECT * FROM " + metric + " WHERE start >= '" + start + "' "
            "and start < '" + end + "' "
        ...
        try:
            ...
            if (not metric == "transfer_function") and (
                not metric == "orientation_check"
            ):
                DF.rename(columns={"value": metric}, inplace=True)
                DF[metric] = DF[metric].map(float)
            DF.drop("lddate", axis=1, inplace=True)
...
def mergeMetricDF(nets, stas, locs, chans, start, end, metrics, metricSource):
    ...
    emptyMets = []
    failedMetrics = list()
    skipTransferFunction = False
    skipOrientationCheck = (
        False  # since multiple 'metrics' can have these metrics, only do it once
    )
    ...
    for metric in metrics:
        ...
        tempDF, failedMetrics = getMetrics(
            nets,
            stas,
            locs,
            chans,
        ...
        if DF.empty:
            ...
            try:
                DF = pd.merge(
                    ...
                    tempDF,
                    how="outer",
                    left_on=["target", "start", "end"],
                    right_on=["target", "start", "end"],
                )
            except:
                print(
                    "ERROR: Something went wrong with the metric. You should try again."
                )
                quit()
