...
[startday, endday, subdir] = reportUtils.calculate_dates(
    reportFrequency
)  # Determines default start and end dates, directory for report
...
groupsDict = {
    ...
    "Metadata": ["horDip", "lowScale", "nonMSUnits", "zDip", "zeroZ"],
    "State of Health": [
        "ampSat",
        "clip",
        "filtChg",
    ...
    "Timing": ["noTime", "poorTQual", "suspectTime"],
}
...
mainDir = "./"
directory = (
    mainDir + network + "/" + subdir + "/"
)  # creates a new directory for each report
...
csvfile = (
    directory + csvfilename
)  # file to write tickets to, used to generate report in GENERATE REPORT tab
