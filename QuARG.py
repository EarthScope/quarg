...
class MainScreen(Screen):
    ...
    def generate_csv(self):
        ...
        try:
            ...
            try:
                selected_tickets = subsettedTickets.drop(
                    ["networks", "stations", "locations", "channels"], axis=1
                ).sort_index(axis=0)
            except:
                try:
                    selected_tickets = pd.DataFrame(columns=allTickets.columns).drop(
                        ["networks", "stations", "locations", "channels"], axis=1
                    )
                except:
            ...
            selected_tickets["target"] = (
                selected_tickets["network"]
                + " "
                + selected_tickets["station"]
                + " "
                + selected_tickets["location"]
                + " "
                + selected_tickets["channel"]
            )
        ...
        sorted_tickets = selected_tickets[
            [
                "id",
                "tracker",
                "target",
        ...
        self.sorted_tickets = sorted_tickets.sort_values(
            by=["tracker", "subject", "target", "start_date"]
        )
        ...
        if os.path.isfile(self.directory + "/" + self.csv):
            ...
            masterDict["_popup"] = Popup(
                title=self.csv, content=content, size_hint=(0.9, 0.9)
            )
            masterDict["_popup"].open()
        else:
            try:
                ...
                self.warning_popup(
                    "WARNING: Could not create directory %s, is a directory specified?"
                    % self.directory
                )
                return
        ...
        if self.generate_csv_state == True:
            ...
            try:
                with open(self.preference) as f:
                    local_dict = locals()
                    exec(
                        compile(f.read(), self.preference, "exec"),
                        globals(),
                        local_dict,
                    )
            except:
            ...
            if os.path.isfile(self.directory + "/" + self.csv):
                self.fileType = self.directory + "/" + self.csv
                content = OverwriteDialog(
                    dontDoIt=self.dismiss_csv_popup, doIt=self.remove_file
                )
                masterDict["_popup"] = Popup(
                    title=self.csv, content=content, size_hint=(0.9, 0.9)
                )
            ...
            try:
                ...
                self.warning_popup(
                    "WARNING: Unable to parse start date, is it formatted correctly?"
                )
                return
            try:
                with open(self.preference) as f:
    ...
    def generate_report_pt2(self):
        ...
        if os.path.isdir(self.zipDir) or os.path.isfile(self.report_fullPath):
            ...
            masterDict["_html_popup"] = Popup(
                title=self.zipDir, content=content, size_hint=(0.9, 0.9)
            )
    ...
    def do_generate(self):
        ...
        if self.start == "" or self.end == "":
            self.warning_popup(
                "WARNING: Start and/or End not provided, report filled in missing values from Preference File\n                      Fill in missing dates and Generate Report again if that was unintended"
            )
        ...
        try:
            ...
            if len(self.generateReport_status) > 0:
                self.warning_popup(
                    "WARNING: There were errors generating the report: \n%s"
                    % self.generateReport_status
                )
            else:
                self.warning_popup(
                    "Report successfully generated in %s" % (self.report_fullPath)
                )
                masterDict["warning_popup"].title = "Success"
        except Exception as e:
            print(e)
            self.warning_popup(
                "WARNING: There was an error running generateHTML.py. Check the command line for more information."
            )
...
class PreferencesScreen(Screen):
    ...
    def go_to_preferencesLayout(self):
        ...
        try:
            ...
            if os.path.dirname(preferenceFile) == os.getcwd():
                preferences_screen.preference_file.text = "./%s" % os.path.basename(
                    preferenceFile
                )
            else:
        ...
        with open(masterDict["thresholds_file"]) as f:
            local_dict = locals()
            exec(
                compile(f.read(), masterDict["thresholds_file"], "exec"),
                globals(),
                local_dict,
            )
        ...
        masterDict["pref_threshold_names"] = sorted(
            list(masterDict["thresholdsDict"].keys())
        )
    ...
    def load_preference_file(self, preferenceFile):
        ...
        if os.path.isfile(preferenceFile):
            try:
                ...
                preferences_screen.pref_ticket.text = masterDict[
                    "preference_ticketFile"
                ]
                ...
                preferences_screen.frequency_btn.text = masterDict[
                    "preference_reportFrequency"
                ]

                preferences_screen.pref_H.text = ",".join(
                    masterDict["preference_chanTypes"]["H"]
                )
                preferences_screen.pref_V.text = ",".join(
                ...
                if masterDict["preference_metricSource"] == "IRIS":
                    ...
                    preferences_screen.metric_source_btn.text = (
                        "Local ISPAQ SQLite Database"
                    )
                ...
                for instrument in [
                    x.strip() for x in masterDict["preference_instruments"]
                ]:
                    if not instrument == "":
                        try:
                            node = masterDict["groupsDict"].index(instrument)
                            preferences_screen.instrument_list_rv._layout_manager.select_node(
                                node
                            )
                            ...
                            print(
                                "INFO: Could not load Instrument Group %s from preference file"
                                % instrument
                            )

                for group in [
                    x.strip() for x in masterDict["preference_thresholdGroups"]
                ]:
                    if not group == "":
                        try:
                            node = masterDict["thresholdGroups"].index(group)
                            preferences_screen.thresholdGroups_list_rv._layout_manager.select_node(
                                node
                            )
    ...
    def pref_file_load(self):
        ...
        masterDict["_popup"] = Popup(
            title="Load preference file", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def thresh_file_load(self):
        ...
        masterDict["_popup"] = Popup(
            title="Load thershold file", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def load_thresh_file(self, path, filename):
        ...
        try:
            if os.path.dirname(filename[0]) == os.getcwd():
                preferences_screen.pref_thresh.text = "./%s" % os.path.basename(
                    filename[0]
                )
            else:
    ...
    def working_dir_load(self):
        ...
        masterDict["_popup"] = Popup(
            title="Load working directory", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def meta_file_load(self):
        ...
        masterDict["_popup"] = Popup(
            title="Load metadata file", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def metric_file_load(self):
        ...
        masterDict["_popup"] = Popup(
            title="Load metric file", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def load_metric_file(self, path, filename):
        ...
        try:
            if os.path.dirname(filename[0]) == os.getcwd():
                preferences_screen.metric_source_text.text = "./%s" % os.path.basename(
                    filename[0]
                )
            else:
    ...
    def warning_popup(self, txt):
        ...
        popupContent.add_widget(Label(size_hint_y=0.5))
        popupContent.add_widget(scrvw)
        popupContent.add_widget(Label(size_hint_y=0.5))
        popupContent.add_widget(returnButton)
    ...
    def confirmation_popup(self):
        ...
        masterDict["_popup"] = Popup(
            title="Success", content=popupContent, size_hint=(0.66, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def save_preference_file(self):
        ...
        self.selected_thresholdGroups = list(
            set(
                [
                    masterDict["thresholdGroups"][x]
                    for x in self.thresholdGroups_selectionIndices
                ]
            )
        )
        self.chanTypes = {
            "H": tuple(preferences_screen.pref_H.text.split(",")),
        ...
        self.groupsList = list(
            set([x.split("::")[0].strip() for x in masterDict["groups_and_names"]])
        )
        ...
        for tmpGroup in self.groupsList:
            ...
            for tmpthresh in [
                x.split("::")[-1].strip()
                for x in masterDict["groups_and_names"]
                if x.startswith(tmpGroup + " :: ")
            ]:
        ...
        if imissing == 1:
            self.warning_popup(
                "WARNING: The following fields are required: %s " % missingList
            )
            return

        if iwarning == 1:
            self.doubleCheck_popup(
                "Just so you know: the following are empty: %s" % warningList
            )
            return
...
"""
            ...
            f.write(
                "network = '%s'\nstation = '%s'\nlocation = '%s'\nchannels = '%s'"
                % (
                    preferences_screen.pref_net.text,
                    preferences_screen.pref_sta.text,
                    preferences_screen.pref_loc.text,
                    preferences_screen.pref_cha.text,
                )
            )
            ...
            f.write(
                "metricSource = '%s'\nmetadataSource = '%s'"
                % (self.metricSource, self.metadataSource)
            )
            ...
            f.write(
                "\nfilename = '%s'        # file to write to in FIND ISSUES tab"
                % preferences_screen.pref_issue.text
            )
            f.write('\nif not subdir == "": ')
...
class ThresholdGroupsScreen(Screen):
    ...
    def go_to_thresholdGroups(self):
        ...
        unused_thresholds = [
            x for x in masterDict["pref_threshold_names"] if x not in used_thresholds
        ]
    ...
    def add_threshGroup(self):
        ...
        if (
            not self.thresholdNames_selectionIndices
            or not self.thresholdGroups_selectionIndices
        ):
            self.warning_popup("WARNING: Must select both threshold Name and Group")
            return

        completed_thresholdName_idx = list()
        for thresholdName_idx in sorted(
            self.thresholdNames_selectionIndices, reverse=True
        ):
            self.thresholdNames_rv._layout_manager.deselect_node(thresholdName_idx)
            ThresholdGroupsScreen.thresholdNames_selectionIndices = [
                v
                for v in ThresholdGroupsScreen.thresholdNames_selectionIndices
                if v != thresholdName_idx
            ]
            ...
            thisGroup = masterDict["thresholdGroups"][
                self.thresholdGroups_selectionIndices[0]
            ]
    ...
    def remove_threshGroup(self):
        ...
        if not self.thresholdGroupAndNames_selectionIndices:
            self.warning_popup(
                "WARNING: Must select at least one Group :: Threshold item to remove it"
            )
            return

        for groupThreshold_idx in sorted(
            self.thresholdGroupAndNames_selectionIndices, reverse=True
        ):
...
class ThresholdsScreen(Screen):
    ...
    def go_to_thresholdsLayout(self):
        ...
        ## Threshold names
        masterDict["threshold_names"] = sorted(
            list(masterDict["thresholdsDict"].keys())
        )
                    ...
                    if line.startswith("metric name"):
                        ...
                        if metric_name == "transfer_function":
                            ...
                            subMets = [
                                ...
                                if x not in ['"lddate"', '"target"', '"start"', '"end"']
                            ]
    ...
    def disable_rvs(self):
        ...
        if (
            self.ids.thresh_ratio_id.state == "down"
            or self.ids.thresh_compare_id.state == "down"
        ):
    ...
    def display_definition(self):
        ...
        if (not self.selected_threshold == []) and (not self.selected_group == []):
            try:
                thisDefinition = [
                    {"text": x}
                    for x in masterDict["thresholdsDict"][self.selected_threshold[0]][
    ...
    def new_threshold_popup(self):
        ...
        for threshold in masterDict["threshold_names"]:
            ...
            btn.bind(
                on_release=lambda btn: self.selectExistingThreshold.select(btn.text)
            )
        ...
        self.selectExistingThreshold.bind(
            on_select=lambda instance, x: setattr(self.thresholdTextInput, "text", x)
        )
        ...
        masterDict["_popup"] = Popup(
            title="Edit Thresholds", content=popupContent, size_hint=(0.66, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def all_thresholds_popup(self, *kwargs):
        ...
        masterDict["_popup"] = Popup(
            title="Existing Threshold Definition",
    ...
    def new_group_popup(self):
        ...
        col1a = BoxLayout(orientation="vertical", size_hint_x=0.5)
        ...
        col3b = BoxLayout(orientation="vertical", size_hint_x=0.5)
        ...
        masterDict["_popup"] = Popup(
            title="Edit Instrument Groups", content=popupContent, size_hint=(0.66, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def set_group_settings(self, instance, txt):
        try:
            ...
            self.netTextInput.text = ",".join(
                masterDict["instrumentGroupsDict"][txt]["network"]
            )
            self.staTextInput.text = ",".join(
                masterDict["instrumentGroupsDict"][txt]["station"]
            )
            self.locTextInput.text = ",".join(
                masterDict["instrumentGroupsDict"][txt]["location"]
            )
            self.chanTextInput.text = ",".join(
                masterDict["instrumentGroupsDict"][txt]["channel"]
            )
    ...
    def add_new_grouping(self, *kwargs):
        ...
        masterDict["instrumentGroupsDict"][newGroup] = {
            "network": theseNets,
            "station": theseStas,
            "location": theseLocs,
            "channel": theseChas,
    ...
    def new_threshold_group_popup(self):
        ...
        for group in masterDict["thresholdGroups"]:
            ...
            btn.bind(
                on_release=lambda btn: self.selectExistingThresholdGroup.select(
                    btn.text
                )
            )
        ...
        self.selectExistingThresholdGroup.bind(
            on_select=lambda instance, x: setattr(
                self.thresholdGroupTextInput, "text", x
            )
        )
        ...
        masterDict["_popup"] = Popup(
            title="Edit Threshold Groups", content=popupContent, size_hint=(0.66, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def remove_defintion_line(self):
        try:
            masterDict["thresholdsDict"][self.selected_threshold[0]][
                self.selected_group[0]
            ] = [
                v
                for v in masterDict["thresholdsDict"][self.selected_threshold[0]][
                    self.selected_group[0]
                ]
                if v != self.selected_definition
            ]
            if (
                masterDict["thresholdsDict"][self.selected_threshold[0]][
                    self.selected_group[0]
                ...
                del masterDict["thresholdsDict"][self.selected_threshold[0]][
                    self.selected_group[0]
                ]
    ...
    def confirm_group_removal_popup(self, *kwargs):
        ...
        warningLabel = Label(
            text="This will remove this Instrument Group. Any thresholds associated with it will be broken. Are you sure?"
        )
        ...
        masterDict["confirm_popup"] = Popup(
            title="Delete?", content=popupContent, size_hint=(0.5, 0.5)
        )
    ...
    def confirm_threshold_removal_popup(self, *kwargs):
        ...
        warningLabel = Label(
            text="This will remove all definitions for this threshold. Are you sure?"
        )
    ...
    def confirm_thresholdGroup_removal_popup(self, *kwargs):
        ...
        warningLabel = Label(
            text="This will remove this Threshold Group. Any metrics associated with it will be broken. Are you sure?"
        )
    ...
    def remove_threshold(self, *kwargs):
        ...
        try:
            masterDict["threshold_names"] = [
                x for x in masterDict["threshold_names"] if x != threshToRemove
            ]
    ...
    def remove_instrument_group(self, *kwargs):
        ...
        try:
            masterDict["instrument_groups"] = [
                x for x in masterDict["instrument_groups"] if x != groupToRemove
            ]
    ...
    def remove_thresholdGroup(self, *kwargs):
        ...
        try:
            masterDict["thresholdGroups"] = [
                x for x in masterDict["thresholdGroups"] if x != threshGroupToRemove
            ]
    ...
    def add_metric_to_threshold(self, *kwargs):
        ...
        if self.ids.horiz_chans_id.state == "down":
            ...
            if threshold_screen.channel_vs_button.state == "down":
                ...
                chanToDo = "%s:%s" % (
                    chanToDo,
                    threshold_screen.channel_avg_button.text,
                )
        ...
        if self.ids.thresh_avg_id.state == "down":
            ...
            if (
                ...
                self.warning_popup(
                    "WARNING: Greater Than, Less Than, and/or Equal To must be selected"
                )
                return

            if (metric_cutoff == "") or not isinstance(metric_cutoff, float):
                self.warning_popup(
                    "WARNING: Metric cutoff value required, must be numeric"
                )
                return
            ...
            if (not prevDef == "") and (not prevDef == []):
                self.warning_popup(
                    "WARNING: Average thresholds can only have one definition"
                )
                return
            ...
            if neq == "down":
                newPart = (
                    newPart + "!"
                )  # this is split so that the "!" can be at the beginning of the operator
            if gr == "down":
                newPart = newPart + ">"
            if le == "down":
                newPart = newPart + "<"
        ...
        # Median
            ...
            if (not prevDef == "") and (not prevDef == []):
                self.warning_popup(
                    "WARNING: Median thresholds can only have one definition"
                )
                return
        ...
        # Ratio
            ...
            if not field_passes:
                ...
                if (
                    ...
                    and (eq == "normal")
                    and (neq == "normal")
                ) or ((metric_cutoff == "") or not isinstance(metric_cutoff, float)):
                    self.warning_popup(
                        "WARNING: Metric name OR operator and numeric value required"
                    )
                    return
            else:
            ...
            if any("average ::" in s for s in prevDef) or any(
                "median ::" in s for s in prevDef
            ):
                self.warning_popup(
                    "WARNING: Average and Median thresholds can only have one definition"
                )
                return
            ...
            if metric == "":
                ...
                if len(indices) < 2:
                    self.warning_popup(
                        "WARNING: Need to specify two metrics before providing the cutoff information"
                    )
                    return
                ...
                if len(indices) >= 2:
                    self.warning_popup(
                        "WARNING: Two metrics already listed - waiting for cutoff value"
                    )
                    return
                ...
                if chanToDo != "":
                    if len(indices) == 0:
                        ...
                        if chanToDo == "V":
                            if prevChanType == "":
                                self.warning_popup(
                                    "WARNING: Unable to compare verticals to all channels"
                                )
                                return
                            if prevChanType == "V":
                                if prevMetric == metric:
                                    self.warning_popup(
                                        "WARNING: Cannot compare verticals to verticals of same metric"
                                    )
                                    return
                                else:
                                    metric = "%s[%s]" % (metric, chanToDo)
                            if prevChanType == "H:vs":
                                self.warning_popup(
                                    "WARNING: H:vs can only be used with another H:vs"
                                )
                                return
                            ...
                            if prevChanType == "":
                                self.warning_popup(
                                    "WARNING: Unable to compare horizontals to all channels"
                                )
                                return
                            if prevChanType == "H":
                                if prevMetric == metric:
                                    self.warning_popup(
                                        "WARNING: Cannot compare horizontals to horizontals of same metric"
                                    )
                                    return
                                else:
                                    metric = "%s[%s]" % (metric, chanToDo)
                            ...
                            if prevChanType != "H:vs":
                                ...
                                if prevMetric != metric:
                                    self.warning_popup(
                                        "WARNING: H:vs can only be used for a single metric"
                                    )
                                    return
                            metric = "%s[%s]" % (metric, chanToDo)
        ...
        # nearly the same as ratio, except uses the operator instead of division
            ...
            if not field_passes:
                ...
                if (
                    ...
                    self.warning_popup(
                        "WARNING: Metric name OR operator must be selected"
                    )
                    return
            else:
                # Then we are simply adding the metric without the operator and cutoff
            ...
            if metric == "":
                ...
                if len(indices) >= 2:
                    self.warning_popup(
                        "WARNING: Two metrics already listed: waiting for operator"
                    )
                    return
        ...
        if not newPart in prevDef:
            ...
            ("[H:vs] :: ratio" in newPart)
            or ("[H:vs] :: compare" in newPart)
            or ("[H:vs]) :: ratio" in newPart)
            or ("[H:vs]) :: compare" in newPart)
        ):
            prevDef.append(newPart)
                ...
                for threshold in masterDict["thresholdsDict"].keys():
                    ...
                    for group in masterDict["thresholdsDict"][threshold].keys():
                        ...
                        for definition in defs:
                            if ("average ::" in definition) or (
                                "median ::" in definition
                            ):
...
class ExamineIssuesScreen(Screen):
    ...
    def load_issueFile(self):
        ...
        try:
            ...
            ExamineIssuesScreen.warning_popup(
                ExamineIssuesScreen,
                "WARNING:  Unable to open Issue File. Either Directory and/or Filename were omitted, or the file does not exist.",
            )
        ...
        currentList = [
            {
                "text": "".join(
                    [
                        x[y]
                        .replace("\n", " ")
                        .ljust(spacing_dict[y])[0 : spacing_dict[y]]
                        for y in range(len(x))
                    ]
                )
            }
            for x in self.currentDF.values.tolist()
        ]
    ...
    def set_target(self):
        try:
            self.target = (
                self.currentSNCL.split(".")[0]
                + "."
                + self.currentSNCL.split(".")[1]
                + ".*.*H*"
            )
        except:
            self.target = ""
    ...
    def see_waveforms(self):
        ...
        if (
            ...
            self.warning_popup(
                "WARNING: All target fields must be specified. Lists are allowed."
            )
            return
                    ...
                    for cha in self.channel.split(","):
                        ...
                        try:
                            urllib.request.urlretrieve(
                                imageURL_complete, image_filename
                            )
    ...
    def see_metric_timeseries(self):
        try:
            ...
            startdate = datetime.date(
                int(startday[0]), int(startday[1]), int(startday[2])
            )
            ...
            for metric in [i.lower().strip() for i in self.metrics.split(",")]:
                masterDict["plot_filename"] = (
                    masterDict["imageDir"]
                    + "tmp.metric_timeseries_"
                    + metric
                    + "_"
                    + datetime.datetime.now().strftime("%Y%m%d%H%M%s")
                    + ".png"
                ...
                if metric != "orientation_check":
                    ...
                    targets_list = [
                        sncl + "-" + field
                        for sncl in metricDF.target.unique()
                        for field in ["azimuth_Y_obs", "azimuth_X_obs", "difference"]
                    ]
                    plotDF = pd.DataFrame(columns=targets_list, index=metricDF["date"])

                    for field in ["azimuth_Y_obs", "azimuth_X_obs"]:
                        for index, row in metricDF.iterrows():
                            plotDF.loc[row["date"], row["target"] + "-" + field] = row[
                                field
                            ]
    ...
    def see_pdfs(self):
        ...
        startdate = datetime.datetime.strptime(self.startday, "%Y-%m-%d").strftime(
            "%Y-%m-01"
        )
        if self.network:
    ...
    def see_nmt(self):
        ...
        if not self.network:
            self.warning_popup(
                "WARNING: Network code required for Noise Mode Timeseries (cannot be wildcarded)"
            )
            return
        if not self.station:
            self.warning_popup(
                "WARNING: Station code required for  Noise Mode Timeseries (cannot be wildcarded)"
            )
            return
        if not self.location:
            self.warning_popup(
                "WARNING: Location code required for  Noise Mode Timeseries (cannot be wildcarded)"
            )
            return
        if not self.channel:
            self.warning_popup(
                "WARNING: Channel code required for  Noise Mode Timeseries (cannot be wildcarded)"
            )
            return
        ...
        nmtURL = (
            ...
            + "&endtime="
            + self.endday
            + "&output=power&format=plot&nodata=404"
        )
    ...
    def see_goat(self):
        ...
        if not self.network:
            self.warning_popup(
                "WARNING: Network code required for GOAT (can be wildcarded)"
            )
            return
        if not self.station:
            self.warning_popup(
                "WARNING: Station code required for GOAT (can be wildcarded)"
            )
            return
        if not self.location:
            self.warning_popup(
                "WARNING: Location code required for GOAT (can be wildcarded)"
            )
            return
        if not self.channel:
            self.warning_popup(
        ...
        enddate = datetime.datetime.strptime(self.endday, "%Y-%m-%d").strftime(
            "%Y/%m/%d"
        )

        goatURL = (
            "http://ds.iris.edu/data_available/"
            + self.network
            + "/"
            + self.station
            + "/"
    ...
    def see_events(self):
        ...
        self.endday = self.endday.split("T")[0]
        eventURL = (
            "https://earthquake.usgs.gov/fdsnws/event/1/query?starttime="
            + self.startday
            + "&endtime="
            + self.endday
            + "&minmag=5.5&orderby=time&format=text&nodata=404"
        )
    ...
    def see_stations(self):
        ...
        stationURL = (
            stationURL + "&level=channel&format=text&includecomments=true&nodata=404"
        )
    ...
    def load_issue_file(self):
        try:
            ...
            if len(self.listOfSncls) == 0:
                ...
                ExamineIssuesScreen.currentDF = self.df[
                    self.df["SN"] == self.currentSNCL
                ].drop(["SN", "NET", "STA", "LOC", "CHA"], axis=1)
                self.update_data()
                ...
                ExamineIssuesScreen.selectionIndices = (
                    list()
                )  # have to clear the selection list if we've switched the displayed issues
                self.deselect_all()
    ...
    def go_to_next(self):
        ...
        if len(self.listOfSncls) == 0:
            ...
            if self.iTodo == 1:
                ExamineIssuesScreen.currentDF = self.df[
                    (self.df["SN"] == self.currentSNCL) & (self.df["STATE"] == "TODO")
                ].drop(["SN", "NET", "STA", "LOC", "CHA"], axis=1)
            else:
                ExamineIssuesScreen.currentDF = self.df[
                    self.df["SN"] == self.currentSNCL
    ...
    def see_all_issues(self):
        self.get_examine_inputs()
        ExamineIssuesScreen.currentDF = self.df.drop(
            ["SN", "NET", "STA", "LOC", "CHA"], axis=1
        )
        self.update_data()
        ...
        if self.iTodo == 1:
            ...
            if self.threshold:
                ExamineIssuesScreen.currentDF = self.df[
                    (
                        self.df["ISSUE"].isin(
                            [x.strip() for x in self.threshold.split(",")]
                        )
                    )
                    & (self.df["STATE"] == "TODO")
                ].drop(["SN", "NET", "STA", "LOC", "CHA"], axis=1)
                ...
                if len(self.listOfSncls) == 0:
                    ...
                    ExamineIssuesScreen.currentDF = self.df[
                        (self.df["SN"] == currentSNCL) & (self.df["STATE"] == "TODO")
                    ].drop(["SN", "NET", "STA", "LOC", "CHA"], axis=1)
                    self.update_data()
    ...
    def see_remaining(self):
        ...
        if self.iRemaining == 1:
            ...
            ExamineIssuesScreen.currentDF = self.df[(self.df["STATE"] == "TODO")].drop(
                ["SN", "NET", "STA", "LOC", "CHA"], axis=1
            )
            self.update_data()
        ...
        try:
            if self.network:
                tmpDF = tmpDF[
                    tmpDF["NET"]
                    .str.upper()
                    .isin([x.strip().upper() for x in self.network.split(",")])
                ]
            if self.station:
                tmpDF = tmpDF[
                    tmpDF["STA"]
                    .str.upper()
                    .isin([x.strip().upper() for x in self.station.split(",")])
                ]
            if self.location:
                tmpDF = tmpDF[
                    tmpDF["LOC"]
                    .str.upper()
                    .isin([x.strip().upper() for x in self.location.split(",")])
                ]
            if self.channel:
                tmpDF = tmpDF[
                    tmpDF["CHA"]
                    .str.upper()
                    .isin([x.strip().upper() for x in self.channel.split(",")])
                ]

            if self.iTodo == 1:
                tmpDF = tmpDF[tmpDF["STATE"] == "TODO"]

            ExamineIssuesScreen.currentDF = tmpDF.drop(
                ["SN", "NET", "STA", "LOC", "CHA"], axis=1
            )
                ...
                if self.iTodo == 1:
                    tmpDF = tmpDF[
                        (
                            ...
                            .str.upper()
                            .isin(
                                [x.strip().upper() for x in self.threshold.split(",")]
                            )
                        )
                        & (tmpDF["STATE"] == "TODO")
                    ]
                    ...
                    tmpDF = tmpDF[
                        tmpDF["ISSUE"]
                        .str.upper()
                        .isin([x.strip().upper() for x in self.threshold.split(",")])
                    ]
    ...
    def see_notes(self):
        ...
        notes_layout = StackLayout(
            orientation="lr-tb", size_hint_y=None, spacing=[0, 20]
        )
        notes_layout.bind(minimum_height=notes_layout.setter("height"))

        for ind in indToChange:
            t = Label(
                text=currentNotes["SNCL"][ind], size_hint_y=None, size_hint_x=0.15
            )
            notes_layout.add_widget(t)

            s = Label(
                text=currentNotes["START"][ind], size_hint_y=None, size_hint_x=0.1
            )
            notes_layout.add_widget(s)

            e = Label(text=currentNotes["END"][ind], size_hint_y=None, size_hint_x=0.1)
            notes_layout.add_widget(e)

            l = Label(
                text=currentNotes["NOTES"][ind], size_hint_y=None, size_hint_x=0.65
            )
        ...
        scrl = ScrollView(size=(Window.width, Window.height * 0.8), size_hint_y=None)
    ...
    def thresholds_popup(self, *kwargs):
        ...
        masterDict["_popup"] = Popup(
            title="Existing Thresholds", content=popupContent, size_hint=(0.4, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def metrics_popup(self, *kwargs):
        ...
        try:
            ...
            print(
                "ERROR: Unable to open metric file %s, does it still exist?"
                % masterDict["metrics_file"]
            )
            return
        ...
        masterDict["_popup"] = Popup(
            title="List of Metrics", content=popupContent, size_hint=(0.2, 0.66)
        )
        masterDict["_popup"].open()
    ...
    def save_progress(self):
        ...
        self.df[
            ["ISSUE", "SNCL", "START", "END", "DAYS", "VALUE", "STATE", "NOTES"]
        ].to_csv(output_file, sep="|", header=False, index=False)
        self.df[
            ["ISSUE", "SNCL", "START", "END", "DAYS", "VALUE", "STATE", "NOTES"]
        ].to_csv(backup_copy, sep="|", header=False, index=False)
        print(
            "Committed to " + output_file + " at " + str(datetime.datetime.now().time())
        )
...
class NewTicketScreen(Screen):
    ...
    def image_popup(self, *kwargs):
        ...
        actionButtons = BoxLayout(
            orientation="vertical", size_hint_x=0.25, padding=10, spacing=5
        )
        ...
        actionButtons.add_widget(
            Button(text="Remove image", on_release=self.remove_images)
        )
        for i in range(4):
        ...
        label2 = Label(
            text="New Caption:", size_hint=(1.0, 1.0), halign="left", valign="middle"
        )
        ...
        captionInputBox.add_widget(
            Button(text="Add caption", on_release=self.add_caption, size_hint_x=0.25)
        )

        captionBox.add_widget(captionLabelBox)
        captionBox.add_widget(captionInputBox)

        navButtons = BoxLayout(
        ...
        navButtons.add_widget(
            Button(text="Return", on_release=self.dismiss_image_popup)
        )
        ...
        masterDict["image_popup"] = Popup(
            title="Select Images",
            content=masterDict["image_content"],
            size_hint=(0.9, 0.9),
        )
    ...
    def remove_images(self, *kwargs):
        ...
        self.ids.image_id.text = "\n".join(
            [i.split("/")[-1] for i in masterDict["imageList"]]
        )
    ...
    def add_caption(self, button, *kwargs):
        try:
            ...
            if str(e) == "list index out of range":
                self.warning_popup(
                    "WARNING: could not add caption to file - no image selected"
                )
            else:
    ...
    def link_popup(self, *kwargs):
        ...
        actionButtons.add_widget(
            Button(text="Remove link", on_release=self.remove_link)
        )
        for i in range(4):
            actionButtons.add_widget(Label())
        ...
        masterDict["link_popup"] = Popup(
            title="Add Links", content=masterDict["link_content"], size_hint=(0.9, 0.9)
        )
    ...
    def remove_link(self, *kwargs):
        for file in self.selectedLinks:
            try:
                masterDict["linkList"] = [
                    v for v in masterDict["linkList"] if v != file
                ]
            except KeyError as e:
                print("WARNING: File not found in list - %s" % e)
    ...
    def import_description(self):
        ...
        if len(set(self.descriptions)) > 1:
            self.warning_popup(
                "WARNING: Multiple Descriptions selected, please select only one for importing"
            )
        else:
    ...
    def clear_thresholds(self):
        ...
        self.selectedThresholds = (
            list()
        )  # this isn't necessary because it will remove them as deselected, but jic
    ...
    def prep_ticket(self, *arg):
        ...
        if (
            ...
            or self.station_str == ""
            or self.channel_str == ""
            or self.location_str == ""
            or self.status_str == ""
            or self.tracker_str == "-"
            or self.status_str == "-"
            or self.category_str == "-"
        ):
            self.warning_popup(
                "WARNING: Required fields: Tracker, Subject, Status, Category, and Target Info (N, S, L, C)"
            )
        else:
            self.do_submit()
...
class UpdateTicketScreen(Screen):
    ...
    def load_ticket_information(self):
        ...
        update_screen.update_tracker_btn.text = SelectedTicketsScreen.selected_ticket[
            "tracker"
        ].item()
        update_screen.update_category_btn.text = SelectedTicketsScreen.selected_ticket[
            "category"
        ].item()
        update_screen.update_status_btn.text = SelectedTicketsScreen.selected_ticket[
            "status"
        ].item()

        update_screen.update_subject.text = SelectedTicketsScreen.selected_ticket[
            "subject"
        ].item()

        update_screen.update_net.text = SelectedTicketsScreen.selected_ticket[
            "network"
        ].item()
        update_screen.update_sta.text = SelectedTicketsScreen.selected_ticket[
            "station"
        ].item()
        update_screen.update_loc.text = SelectedTicketsScreen.selected_ticket[
            "location"
        ].item()
        update_screen.update_cha.text = SelectedTicketsScreen.selected_ticket[
            "channel"
        ].item()
        update_screen.update_description.text = SelectedTicketsScreen.selected_ticket[
            "description"
        ].item()
        update_screen.update_start.text = SelectedTicketsScreen.selected_ticket[
            "start_date"
        ].item()
        update_screen.update_end.text = SelectedTicketsScreen.selected_ticket[
            "end_date"
        ].item()
        ...
        update_screen.update_image.text = "\n".join(
            [
                i.split("/")[-1]
                for i in SelectedTicketsScreen.selected_ticket["images"]
                .item()
                .split(";;;;")
            ]
        )
        ...
        update_screen.update_links.text = "\n".join(
            [
                i
                for i in SelectedTicketsScreen.selected_ticket["links"]
                .item()
                .split(";;;;")
            ]
        )
        masterDict["linkList"] = (
            SelectedTicketsScreen.selected_ticket["links"].item().split(";;;;")
        )

        update_screen.update_last_updated.text = (
            "Last Updated: "
            + SelectedTicketsScreen.selected_ticket["updated"].item().split(".")[0]
        )

        masterDict["imageList"] = dict()
        imagesList = (
            SelectedTicketsScreen.selected_ticket["images"].item().split(";;;;")
        )
        captionsList = (
            SelectedTicketsScreen.selected_ticket["caption"].item().split(";;;;")
        )
        for i in range(len(imagesList)):
            thisImCap = {imagesList[i]: captionsList[i]}
            masterDict["imageList"].update(thisImCap)

        thisTicket = masterDict["tickets"][
            masterDict["tickets"]["id"]
            == int(SelectedTicketsScreen.selected_ticket["id"].item())
        ]
        ...
        for threshold in [
            x.strip()
            for x in SelectedTicketsScreen.selected_ticket["thresholds"]
            .item()
            .split(",")
        ]:
            if not threshold == "":
                ...
                update_screen.updateTicket_thresholds_rv._layout_manager.select_node(
                    node
                )
    ...
    def add_link(self, button, *kwargs):
        try:
            masterDict["linkList"].append(self.linkInput.text)
            self.ids.update_links_id.text = "\n".join(
                [i for i in masterDict["linkList"]]
            )
            print("INFO: Link added")
        except Exception as e:
            print("WARNING: could not add link to list -- %s" % e)
    ...
    def prep_ticket(self, *arg):
        ...
        if (
            ...
            self.warning_popup(
                "Required fields: Tracker, Subject, Status, Category, and Target Fields (N,S,L,C)"
            )
            return False
        else:
            return True
    ...
    def delete_confirmation(self):
        ...
        masterDict["_popup"] = Popup(
            title="Confirm Delete", content=content, size_hint=(0.9, 0.9)
        )
        masterDict["_popup"].open()
    ...
    def do_update(self):
        ...
        if iDoUpdate:
            ...
            conn.close()
            self.ids.update_last_updated_id.text = "Last Updated: " + now.strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            self.confirmation_popup()
    ...
    def go_to_selectedTickets(self):
        ...
        try:
            try:
                ...
                masterDict["ticket_order"] = (
                    tickets_screen.ticket_order.text.lower().replace(" ", "_")
                )
        ...
        header_list = [
            header_list[y].ljust(spacing_dict[y])[0 : spacing_dict[y]]
...
class issues_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        self.index = index
        return super(issues_SelectableLabel, self).refresh_view_attrs(rv, index, data)
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            try:
                ...
                ExamineIssuesScreen.selectionIndices = [
                    v for v in ExamineIssuesScreen.selectionIndices if v != index
                ]
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            try:
                # Can't just do a simple .remove(index) because it'll build up multipl of
                # the same index, and remove only removes one of them - leads to bad markings
                PreferencesScreen.instrument_selectionIndices = [
                    v
                    for v in PreferencesScreen.instrument_selectionIndices
                    if v != index
                ]
...
class preference_thresholdGroups_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def refresh_view_attrs(self, rv, index, data):
        ...
        self.index = index
        return super(
            preference_thresholdGroups_SelectableLabel, self
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            try:
                # Can't just do a simple .remove(index) because it'll build up multipl of
                # the same index, and remove only removes one of them - leads to bad markings
                PreferencesScreen.thresholdGroups_selectionIndices = [
                    v
                    for v in PreferencesScreen.thresholdGroups_selectionIndices
                    if v != index
                ]
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            try:
                # Can't just do a simple .remove(index) because it'll build up multipl of
                # the same index, and remove only removes one of them - leads to bad markings
                ThresholdGroupsScreen.thresholdGroups_selectionIndices = [
                    v
                    for v in ThresholdGroupsScreen.thresholdGroups_selectionIndices
                    if v != index
                ]
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            try:
                if (
                    index
                    not in ThresholdGroupsScreen.thresholdGroupAndNames_selectionIndices
                ):
                    ThresholdGroupsScreen.thresholdGroupAndNames_selectionIndices.append(
                        index
                    )
            except:
                pass
            ...
            try:
                # Can't just do a simple .remove(index) because it'll build up multipl of
                # the same index, and remove only removes one of them - leads to bad markings
                ThresholdGroupsScreen.thresholdGroupAndNames_selectionIndices = [
                    v
                    for v in ThresholdGroupsScreen.thresholdGroupAndNames_selectionIndices
                    if v != index
                ]
            except:
                pass
        ...
        if is_selected:
            try:
                ThresholdsScreen.selected_threshold = [
                    masterDict["threshold_names"][index]
                ]
            except Exception as e:
                print("WARNING: %s" % e)
        else:
            try:
                ThresholdsScreen.selected_threshold = [
                    v
                    for v in ThresholdsScreen.selected_threshold
                    if v != masterDict["threshold_names"][index]
                ]
            except Exception as e:
                ThresholdsScreen.selected_threshold = []
        ...
        if is_selected:
            try:
                ThresholdsScreen.selected_group = [
                    masterDict["instrument_groups"][index]
                ]
            except Exception as e:
                print("WARNING: %s" % e)
        else:
            try:
                ThresholdsScreen.selected_group = [
                    v
                    for v in ThresholdsScreen.selected_group
                    if v != masterDict["instrument_groups"][index]
                ]
            except Exception as e:
                ThresholdsScreen.selected_group = []
        ThresholdsScreen.display_definition(ThresholdsScreen)
...
class threshold_definition_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            try:
                ThresholdsScreen.selected_definition = masterDict["thresholdsDict"][
                    ThresholdsScreen.selected_threshold[0]
                ][ThresholdsScreen.selected_group[0]][index]
            except Exception as e:
                print(e)
        else:
...
class metrics_list_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            ThresholdsScreen.selected_metric = [
                v
                for v in ThresholdsScreen.selected_metric
                if v != masterDict["metrics"][index]
            ]
            try:
...
class metadata_list_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            ...
            ThresholdsScreen.selected_metadata = [
                v
                for v in ThresholdsScreen.selected_metadata
                if v != masterDict["metadata"][index]
            ]
            try:
                threshold_screen.metric_text.text = ThresholdsScreen.selected_metadata[
                    0
                ]
            except:
                threshold_screen.metric_text.text = ""
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            try:
                ...
                SelectedTicketsScreen.selected_ticket = (
                    SelectedTicketsScreen.theseTickets.iloc[[index]]
                )
...
class create_ticket_threshold_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            try:
                NewTicketScreen.selectedThresholds.append(
                    masterDict["threshold_names"][index]
                )
            except:
                pass
            ...
            try:
                ...
                NewTicketScreen.selectedThresholds = [
                    v
                    for v in NewTicketScreen.selectedThresholds
                    if v != masterDict["threshold_names"][index]
                ]
            except:
                pass
...
class update_ticket_threshold_SelectableLabel(RecycleDataViewBehavior, Label):
    ...
    def apply_selection(self, rv, index, is_selected):
        ...
        if is_selected:
            try:
                UpdateTicketScreen.selectedThresholds.append(
                    masterDict["threshold_names"][index]
                )
            except:
                pass
            ...
            try:
                ...
                UpdateTicketScreen.selectedThresholds = [
                    v
                    for v in UpdateTicketScreen.selectedThresholds
                    if v != masterDict["threshold_names"][index]
                ]
            except:
                pass
...
if os.path.exists(database):
    ...
    checkTableSQL = (
        "SELECT count(name) FROM sqlite_master WHERE type='table' AND name='tickets';"
    )
