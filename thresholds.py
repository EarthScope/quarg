...
def do_threshold(
    ...
    def compare_threshold(chanMetricDF, chanMetaDF, subDF):
        ...
        if CH1 == CH2 and chType1 == chType2 == "":
            dfToUse = do_comparison(
                dfToUse, dfToUse[met1], operator, dfToUse[met2], doAbs1, doAbs2
            )
    ...
    for group in threshDefs.keys():
        ...
        if group in instruments:
            ...
            if (len(instDef) > 1) and any("average :: " in s for s in instDef):
                print(
                    "WARNING: thresholds with 'ratio' cannot have multiple parts, skipping"
                )
                continue
                                ...
                                for sta2 in specificSNCL["station"]:
                                    ...
                                    for loc2 in specificSNCL["location"]:
                                        ...
                                        for cha2 in specificSNCL["channel"]:
                                            ...
                                            netMetricDF = netMetricDF[
                                                ~netMetricDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                            netMetaDF = netMetaDF[
                                                ~netMetaDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                ...
                for sta in instrumentGroupsDict[group]["station"]:
                                    ...
                                    for loc2 in specificSNCL["location"]:
                                        ...
                                        for cha2 in specificSNCL["channel"]:
                                            ...
                                            staMetricDF = staMetricDF[
                                                ~staMetricDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                            staMetaDF = staMetaDF[
                                                ~staMetaDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                del specificSNCLs[idx]
                    ...
                    for loc in instrumentGroupsDict[group]["location"]:
                        if loc == "*" or loc == "%" or loc == "":
                                ...
                                if not specificSNCL["location"] == ["*"]:
                                    ...
                                    for loc2 in specificSNCL["location"]:
                                        ...
                                        for cha2 in specificSNCL["channel"]:
                                            ...
                                            locMetricDF = locMetricDF[
                                                ~locMetricDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                            locMetaDF = locMetaDF[
                                                ~locMetaDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                    del specificSNCLs[idx]
                            ...
                            locMetricDF = staMetricDF[
                                staMetricDF["location"].str.contains(loc)
                            ]
                            locMetaDF = staMetaDF[
                                staMetaDF["location"].str.contains(loc)
                            ]
                        ...
                        for chan in instrumentGroupsDict[group]["channel"]:
                            if chan == "*" or chan == "%" or chan == "":
                                ...
                                for idx, specificSNCL in enumerate(specificSNCLs):
                                    if not specificSNCL["channel"] == ["*"]:
                                        ...
                                        for cha2 in specificSNCL["channel"]:
                                            ...
                                            chanMetaDF = chanMetaDF[
                                                ~chanMetaDF["target"].str.contains(
                                                    thisTarget, regex=True
                                                )
                                            ]
                                        del specificSNCLs[idx]
                                ...
                                chanMetricDF = locMetricDF[
                                    locMetricDF["channel"].str.startswith(chan)
                                ]
                                chanMetaDF = locMetaDF[
                                    locMetaDF["channel"].str.startswith(chan)
                                ]
                                ...
                                if "average ::" in subDef:
                                    ...
                                    try:
                                        try:
                                            ...
                                            print(
                                                "WARNING: could not split definition - %s"
                                                % e
                                            )

                                        if (pos3 in metricList) or (
                                            pos3 in metadataList
                                        ):
                                            chanMetricDF, chanMetaDF, itype = (
                                                compare_threshold(
                                                    chanMetricDF, chanMetaDF, subDef
                            ...
                            if hasMetrics == True:
                                if not itype == "average":
                                    chanMetricDF["start"] = pd.to_datetime(
                                        chanMetricDF["start"]
                                    )
                                    chanMetricDF["end"] = pd.to_datetime(
                                        chanMetricDF["end"]
                                    )
                                ...
                                if chanMetricDF.empty or chanMetaDF.empty:
                                    ...
                                    for index, row in chanMetaDF.iterrows():
                                        ...
                                        thisSet = chanMetricDF[
                                            chanMetricDF["target"].str.contains(
                                                complexTarget, regex=True
                                            )
                                        ]
                                        ...
                                        dfs_to_concat = [
                                            df
                                            for df in [finalDF, thisSet]
                                            if not df.empty
                                            and df.dropna(axis=1, how="all").shape[1]
                                            > 0
                                        ]
                                        if dfs_to_concat:
                                            finalDF = pd.concat(dfs_to_concat)
                                        # finalDF = pd.concat([finalDF, thisSet])
                                finalDF = finalDF.drop_duplicates(
                                    subset=["target", "start", "end"]
                                )

                                issues = reportUtils.sortIssueFile(
                                    finalDF, threshold, itype
                                )
                            else:
